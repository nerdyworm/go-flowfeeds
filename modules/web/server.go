...
func Run(options ServerOptions) {
	...
	apiRouter.HandleFunc("/episodes/{id}", Default(episodes.ToggleFavoriteEpisode)).Methods("PUT")
	apiRouter.HandleFunc("/teasers/{id}", Default(episodes.ToggleFavoriteEpisode)).Methods("PUT")
